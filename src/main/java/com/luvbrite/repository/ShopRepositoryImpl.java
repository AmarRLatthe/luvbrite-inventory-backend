...
public class ShopRepositoryImpl implements IShopRepository {
	...
	public int saveShop(CreateShopDTO shopDTO) {
		try {
			...
			userAddQry.append("INSERT INTO user_details ")
					...
					.append("VALUES ( '").append(shopDTO.getFullName()).append("' , '").append(shopDTO.getEmail())
					.append("' , '").append(password).append("' , '").append(shopDTO.getUserName()).append("' , ")
					.append(shopDTO.getOwnerId()).append(" , ").append(shopid.toString()).append(" , ")
					.append(shopDTO.getUserTypeId()).append(" , ").append(shopDTO.getCreatedBy())
					.append(" ) RETURNING id");
	...
	public CreateShopDTO getByShopName(String shopName) {
		try {
			return jdbcTemplate.queryForObject("SELECT * FROM SHOPS WHERE LOWER(shop_name) = LOWER(?)",
					new Object[] { shopName }, new RowMapper<CreateShopDTO>() {
	...
	public int countShopsByShopName(String shopName) {
		try {
			return jdbcTemplate.queryForObject("SELECT COUNT(*) FROM SHOPS WHERE LOWER(shop_name) = LOWER(?)",
					new Object[] { shopName }, Integer.class);
		} catch (Exception e) {
			log.error("message is {} and Exception is {}", e.getMessage(), e);
			return -1;
		}
	...
	public int countShopsByDomain(String domain) {
		try {
			return jdbcTemplate.queryForObject("SELECT COUNT(*) FROM SHOPS WHERE LOWER(domain) = LOWER(?)",
					new Object[] { domain }, Integer.class);
		} catch (Exception e) {
			log.error("message is {} and Exception is {}", e.getMessage(), e);
			return -1;
		}
	...
	public int countShopsByShopNameNId(Integer id, String shopName) {
		try {
			return jdbcTemplate.queryForObject(
					"SELECT COUNT(*) FROM SHOPS WHERE LOWER(shop_name) = LOWER(?) AND id!= ? ",
					new Object[] { shopName, id }, Integer.class);
		} catch (Exception e) {
			log.error("message is {} and Exception is {}", e.getMessage(), e);
			return -1;
		}
	...
	public int countShopsByDomainNId(Integer id, String domain) {
		try {
			return jdbcTemplate.queryForObject("SELECT COUNT(*) FROM SHOPS WHERE LOWER(domain) = LOWER(?) AND id!= ? ",
					new Object[] { domain, id }, Integer.class);
		} catch (Exception e) {
			log.error("message is {} and Exception is {}", e.getMessage(), e);
			return -1;
		}
	...
	public int updateShopById(Integer id, ShopDTO shop) {
		try {
			...
			updateShopQry.append(" UPDATE shops ").append(" SET ").append(" shop_name= ?,  ").append(" domain=? ")
					.append(" WHERE id=? ");

			StringBuilder updateShopOwnerDetailsQry = new StringBuilder();
			updateShopOwnerDetailsQry.append("UPDATE user_details ").append(" SET ").append(" email=?, ")
					.append(" username=?,  ").append(" name=? ").append(" WHERE id = (").append(" SELECT ")
					.append(" SHOPS.shop_admin_id ").append(" FROM SHOPS  ").append(" WHERE ").append(" SHOPS.id = ?)");

			int updateShop = jdbcTemplate.update(updateShopQry.toString(), shop.getShopName(), shop.getDomain(), id);
			if (updateShop > 0) {
				updateShop = jdbcTemplate.update(updateShopOwnerDetailsQry.toString(), shop.getEmail(),
						shop.getShopOwnerUsername(), shop.getShopOwnerName(), id);
			}
			return updateShop;
		} catch (Exception e) {
			log.error("message is {} and Exception is {}", e.getMessage(), e);
	...
	public int updatePwdByshopId(Integer id, String password) {
		try {
			...
			return jdbcTemplate.update(
					"UPDATE user_details	SET password=? 	WHERE id=(SELECT shop_admin_id FROM SHOPS WHERE id=?)",
					newPwd.toString(), id);
		} catch (Exception e) {
			log.error("Message is {} and Exception is {}", e.getMessage(), e);
			return -1;
	...
	public List<String> getAllShopNames() {
		try {
			return jdbcTemplate.query("SELECT DISTINCT(SHOPS.shop_name) FROM SHOPS", new RowMapper<String>() {
				@Override
	...
	@Override
	public List<String> getAllAllowedShopNamesById(Integer id) {
		try {
			StringBuilder qry = new StringBuilder();
			qry.append(" SELECT DISTINCT(SHOPS.shop_name) ")
			.append(" FROM SHOPS   ")
			.append(" JOIN MANAGER_SHOP_ACCESS_DETAILS ON SHOPS.id = MANAGER_SHOP_ACCESS_DETAILS.shop_id  ")
			.append(" WHERE ")
			.append(" MANAGER_SHOP_ACCESS_DETAILS.manager_id=? ") ;
	...
	@Override
	public List<Integer> getIdsFromShopsNames(String[] authorities) {
		try {
			StringBuilder qry = new StringBuilder();
			int total = authorities.length;       
			qry.append(" SELECT ")
			.append(" id ")
			.append(" FROM ")
			.append(" shops ")
			...
			List<Integer> permissionIds = jdbcTemplate.query(qry.toString(),authorities, new RowMapper<Integer>() {
				@Override
